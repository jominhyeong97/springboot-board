...
//    페이징처리를 위한 데이터 요청형식 : 8080/post/list?page=0&size=20&sort=title,desc
    @GetMapping("/list")
    public ResponseEntity<?> postList(@PageableDefault(size=10, sort = "id", direction = Sort.Direction.ASC)Pageable pageable, PostSearchDto postSearchDto) {
        Page<PostListDto> dto = postService.findAll(pageable,postSearchDto);
